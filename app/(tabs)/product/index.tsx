...
export default function ProductScreen() {
  ...
  const renderProduct = ({ item }: { item: Product }) => (
    <TouchableOpacity
      className="w-[48%] bg-white rounded-lg mb-4 shadow-sm overflow-hidden"
    ...
    >
      ...
      <View className="p-3">
        <View className="flex-row justify-between items-center mb-1.5">
          <Text className="text-sm font-semibold text-orange-500 flex-1">
        ...
        <Text className="text-xs font-semibold text-blue-500 mb-1 uppercase">
          {item.brand}
        </Text>

        <Text
          className="text-xs text-gray-800 leading-3.5 mb-1.5 min-h-[42px]"
          numberOfLines={3}
        >
          {item.name}
        </Text>
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="flex-row justify-between items-center px-4 py-3 bg-white">
        <Text className="text-xl font-bold text-gray-800">Sản phẩm</Text>
      ...
      {!searchQuery && (
        ...
        >
          {brands.map((brand) => (
            <TouchableOpacity
              key={brand}
              className={`px-4 py-2 rounded-full border h-fit ${
                selectedBrand === brand
            ...
            >
              <Text
                className={`text-xs font-medium h-7 ${
                  selectedBrand === brand ? "text-blue-500" : "text-gray-600"
                }`}
              >
                {brand}
              </Text>
